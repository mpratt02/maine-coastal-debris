...
library("dplyr")
beach_csv <- read.csv("data/MDMAP_Export_20230827.csv")
...
# Filter out columns that are not important for this exercise
debris_data <- subset(full_table, select=valid_columns)

# Merge and sum rows that have the same date and location.
debris_by_date <- debris_data %>% group_by(
  shoreline_site_id,
  survey_date,
  shoreline_site_name,
  transect_width,
...
# Split the Year into its own column


write.csv(debris_by_date, "data/debris_data.csv")
